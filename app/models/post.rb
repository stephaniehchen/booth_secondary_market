class Post < ApplicationRecord
  ...
  validates :user_id, :uniqueness => { :scope => [:event_id, :active, :sell_buy], :message => "No duplicate posts allowed per user for an event" }
  ...
  validates :event_id, :presence => { :message => "Post must be for a specific event" }
